...
export type UsePostsAnimationParams = {
  topInset: number;
...
export const usePostsAnimation = ({
  ...
  }, [topInset, horizontalPadding, headerTopPadding, targetX, targetY, targetW, targetH]);
  ...
  useAnimatedReaction(
    ...
    (current, previous) => {
      ...
      const crossedReveal =
        previous < INTRO_HEADER_LOGO_REVEAL_AT && current >= INTRO_HEADER_LOGO_REVEAL_AT;
      if (crossedReveal) {
        runOnJS(revealHeaderLogo)();
      }
    },
  ...
  const logoStyle = useAnimatedStyle(() => {
    ...
    if (p <= 1) {
      const w = interpolate(p, [0, 1], [SPLASH_LOGO_START_SIZE, SPLASH_LOGO_SIZE]);
      const h = w;
      return {
        left: (ww - w) / 2,
        top: (wh - h) / 2,
    ...
    const w = interpolate(p, [1, INTRO_PROGRESS_MOVE_END], [SPLASH_LOGO_SIZE, targetW.value]);
    const h = interpolate(p, [1, INTRO_PROGRESS_MOVE_END], [SPLASH_LOGO_SIZE, targetH.value]);
    const startLeft = (ww - SPLASH_LOGO_SIZE) / 2;
    const startTop = (wh - SPLASH_LOGO_SIZE) / 2;
    const left = interpolate(p, [1, INTRO_PROGRESS_MOVE_END], [startLeft, targetX.value]);
    const top = interpolate(p, [1, INTRO_PROGRESS_MOVE_END], [startTop, targetY.value]);

    const overlayOpacity =
      p < INTRO_OVERLAY_FADE_FROM
        ? 1
        : interpolate(p, [INTRO_OVERLAY_FADE_FROM, INTRO_PROGRESS_MOVE_END], [1, 0]);
  ...
  const headerStyle = useAnimatedStyle(() => {
    const p = progress.value;
    return {
      opacity: interpolate(p, [0, 1, 1.12, INTRO_PROGRESS_MOVE_END], [0, 0, 0.45, 1]),
      transform: [
        {
          translateY: interpolate(p, [1, INTRO_PROGRESS_MOVE_END], [HEADER_ENTRY_OFFSET, 0]),
        },
      ],
    };
  });
